#!/usr/bin/env python3
"""Three-camera RGB-D with explicit optical TF and cached parametric bracket CAD."""
import argparse
import copy
import hashlib
import json
...
from build_camera_rig import ROOT, CAMERA_URDF, SCENE, build, output_paths, FIXED_STEP_GEOMETRIES
...
class CameraRig:
    def __init__(self,width=640,height=480,config_path=None):
        ...
        self.urdf_path,self.scene_path,manifest_path=output_paths(self.config_path)
        fresh=False
        if manifest_path.exists() and self.urdf_path.exists() and self.scene_path.exists():
            ...
            fresh=(r['config_sha256']==sha(self.config_path) and r['producer_sha256']==sha(ROOT/'scripts/build_camera_rig.py')
                   and r['source_urdf_sha256']==sha(ROOT/self.config['source_urdf'])
                   and r['urdf_sha256']==sha(self.urdf_path) and r['scene_sha256']==sha(self.scene_path))
            if fresh and self.config.get('bracket_geometry') in ('parametric_photo_reference_v1','parametric_vertical_v2','parametric_side_mount_v3','parametric_side_bend_v4'):
                from bracket_assets import ensure_bracket
                for side,params in self.config['wrists'].items():
                    asset=ensure_bracket(params['length_mm'],params['plate_tilt_deg'],self.config['bracket_geometry'])
                    if r.get('bracket_assets',{}).get(side,{}).get('mesh_sha256')!=sha(asset['mesh']):fresh=False
            if fresh and self.config.get('bracket_geometry') in FIXED_STEP_GEOMETRIES:
                for side,params in self.config['wrists'].items():
                    asset_path=ROOT/params['bracket_manifest']
                    asset=json.loads(asset_path.read_text())
                    cached=r.get('bracket_assets',{}).get(side,{})
                    if cached.get('manifest_sha256')!=sha(asset_path) or cached.get('mesh_sha256')!=sha(ROOT/asset['mesh_m']):fresh=False
        ...
        mutable=set() if self.config.get('parameter_editable') is False else {f'{side}_wrist_camera_{stage}_frame' for side in ('left','right') for stage in ('rod_end','plate')}
    ...
    def set_mount(self,side,length_mm,tilt_deg):
        if self.config.get('parameter_editable') is False:
            raise ValueError('Supplied STEP is fixed geometry; editing a camera transform alone would detach it from its mounting holes.')
        if side not in ('left','right'):raise ValueError('side must be left or right')
        length_mm=float(length_mm);tilt_deg=float(tilt_deg)
        limits=self.config.get('parameter_limits',{'length_mm':[20,150],'plate_tilt_deg':[-30,75]})
        if not limits['length_mm'][0]<=length_mm<=limits['length_mm'][1] or not limits['plate_tilt_deg'][0]<=tilt_deg<=limits['plate_tilt_deg'][1]:
            raise ValueError('Mount parameters outside the CAD range')
        if self.config.get('bracket_geometry') in ('parametric_photo_reference_v1','parametric_vertical_v2','parametric_side_mount_v3','parametric_side_bend_v4'):
            if self.config['wrists'][side]['length_mm']==length_mm and self.config['wrists'][side]['plate_tilt_deg']==tilt_deg:return
            updated=copy.deepcopy(self.config)
            updated['wrists'][side]['length_mm']=length_mm;updated['wrists'][side]['plate_tilt_deg']=tilt_deg
            text=json.dumps(updated,sort_keys=True,indent=2)
            key=hashlib.sha256(text.encode()).hexdigest()[:16]
            cached=ROOT/'outputs/cameras/runtime_configs'/f'{key}.json'
            cached.parent.mkdir(parents=True,exist_ok=True)
            if not cached.exists():cached.write_text(text+'\n')
            # Validate/rebuild actual solids and a separate scene before
            # replacing the live renderer. A new angle rebuilds root gussets.
            build(cached)
            tick=self.tick
            joints={mujoco.mj_id2name(self.model,mujoco.mjtObj.mjOBJ_JOINT,j):float(self.data.qpos[self.model.jnt_qposadr[j]]) for j in range(self.model.njnt)}
            self.close()
            self.__init__(self.width,self.height,cached)
            self.tick=tick
            for name,value in joints.items():
                j=mujoco.mj_name2id(self.model,mujoco.mjtObj.mjOBJ_JOINT,name)
                if j>=0:self.data.qpos[self.model.jnt_qposadr[j]]=value
            mujoco.mj_forward(self.model,self.data)
            return
        ...
        self.model.body_pos[end]=np.array(cfg.get('extension_origin_m',[0,0,0]))+np.array(cfg['extension_axis'])*length_mm*.001
