#!/usr/bin/env python3
"""V2: uninterrupted saddle, radial +Z stem, and directly attached tilted plate.

V1 remains frozen. The former +Y horizontal arm and both large access-relief
...
DEFAULT_OUTPUT=ROOT/"design/wrist_camera_bracket_v2"
START_Z=35.8
V=cq.Vector
box=v1.box
clean=v1.clean
...
def make_base(notes):
    ...
    low,high=math.radians(22),math.radians(158)
...
def build(args):
    ...
    if not whole.isValid() or len(whole.Solids())!=1:
        raise ValueError("V2 whole bracket must be one valid connected solid")
    for name,shape in parts.items():
        if not shape.isValid() or len(shape.Solids())!=1:
            raise ValueError(f"V2 part {name} must be one valid solid")
    whole_volume=volume(whole)
    partition=sum(volume(part) for part in parts.values())
    if abs(partition-whole_volume)>1e-3:raise ValueError("V2 component partition failed")
    ...
    if not step_check["valid"] or step_check["solid_count"]!=1 or step_check["volume_absolute_error_mm3"]>1e-3:
        raise ValueError("V2 exported STEP failed reimport")
    if not mm_check["is_volume"] or not metric_check["is_volume"] or max(mm_check["relative_volume_error"],metric_check["relative_volume_error"])>.005:
        raise ValueError("V2 exported STL failed closed-volume verification")
    records={}
    for name,shape in parts.items():
    ...
    report={
        "schema":"wrist_camera_bracket_radial_stem_v2",
        "design_basis":"user V2 changes: remove both large relief holes, delete the complete +Y arm, extend the +Z stem directly to the plate",
        ...
        "coordinate_convention":{"units":"mm except *_m.stl and explicit SI fields","root":"original hand-adapter shoulder datum","axes":{"X":"across wrist","Y":"wrist axis toward fingers","Z":"radially outward and along the V2 upright"},"theta":"atan2(Z,X), positive equals right-handed Ry(-theta)"},
